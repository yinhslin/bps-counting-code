...
ReducedExact[charges_,degree_,NN_]:=Module[{level, indexact,AllQTerm,chunk,batches,filename,reducedTerm,ind,AllRTerms,Rmatrix,RVector,SimpRVector},
	level = charges . levelvector;
	...
	Print["Spliting jobs"];
	chunk = 1500000;
	batches = Partition[AllQTerm, UpTo[chunk]];
	Print["chunk = ", chunk, "  batches = ", Length@batches];
	Print["Computing Abelianization"];
	Do[
		reducedExact[charges,degree,NN] = Abelianize[batches[[i]]];
		filename = reducedExactDirectory<>ToString[level]<>"_"<>StringRiffle[ToString[#]&/@charges,"_"]<>"_"<>ToString[degree]<>"_"<>ToString[NN]<>"-"<>ToString[i]<>".mx";
		DumpSave[filename,reducedExact];
	...
	Do[
		filename = reducedExactDirectory<>ToString[level]<>"_"<>StringRiffle[ToString[#]&/@charges,"_"]<>"_"<>ToString[degree]<>"_"<>ToString[NN]<>"-"<>ToString[i]<>".mx";
		Get[filename];
		reducedTerm = Join[reducedTerm,reducedExact[charges,degree,NN]];
		ClearAll[reducedExact];
	,
		{i,1,Length[batches]}
	];
	...
	filename = reducedExactDirectory<>ToString[level]<>"_"<>StringRiffle[ToString[#]&/@charges,"_"]<>"_"<>ToString[degree]<>"_"<>ToString[NN]<>"-temp.mx";
	If[!FileExistsQ[filename],
		ind = indexact[[1]];
		...
		reducedExactTemp[charges,degree,NN] = {ind, Rmatrix, AllRTerms};
		DumpSave[filename,reducedExactTemp];
		ClearAll[reducedExactTemp,AllRTerms,reducedTerm];
	,
		Print["Loading back ind and Rmatrix"];
		Print["Memory Available: ",MemoryAvailable[]/2^30//N," GB"];
		Get[filename];
		ind = reducedExactTemp[charges,degree,NN][[1]];
		Rmatrix = reducedExactTemp[charges,degree,NN][[2]];
		ClearAll[reducedExactTemp];
	];
	...
	Print["Memory Available: ",MemoryAvailable[]/2^30//N," GB"];
	Get[filename];
	AllRTerms = reducedExactTemp[charges,degree,NN][[3]];
	Print["Memory Available: ",MemoryAvailable[]/2^30//N," GB"];
...
Exec[] := Module[{},
	filename = reducedExactDirectory<>ToString[level]<>"_"<>StringRiffle[ToString[#]&/@charges,"_"]<>"_"<>ToString[degree]<>"_"<>ToString[NN]<>".mx";
	If[!FileExistsQ[filename],
		ClearAll[exact];
		TimeConstrained[
			Check[
				reducedExact[charges,degree,NN] = ReducedExact[charges,degree,NN];
			,
			...
			DumpSave[filename,reducedExact];
			Print["Saved result"];
			tmp = reducedExact[charges,degree,NN];
			ClearAll[reducedExact];
			Get[filename];
			If[tmp =!= reducedExact[charges,degree,NN],
				DeleteFile[filename];
				,
